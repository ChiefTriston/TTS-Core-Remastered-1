...
def run(context):
    ...
    for speaker_id in speaker_ids:
        ...
        for idx, tag in enumerate(tags):
            corrected = st.selectbox(f"Slice {idx}: {tag['label']}", all_emotions, index=all_emotions.index(tag['label']) if tag['label'] in all_emotions else 0)
            if corrected != tag['label']:
                feedback = {'slice': idx, 'correction': corrected, 'rule_id': tag['rule_id']}
                update_emotion_rules(feedback, rules_path)
