...
def run(context):
    ...
    for speaker_id in speaker_ids:
        ...
        with open(os.path.join(speaker_out, 'drift_log.json'), 'r') as f:
            ...
            log = json.load(f)
            portalocker.unlock(f)
        confidence_drift_slope.append(log['confidence_drift_slope'])
        emotion_entropy.append(log['emotion_entropy'])
    
    confidence_drift_slope = np.mean(confidence_drift_slope)
    emotion_entropy = np.mean(emotion_entropy)
    
    with open(os.path.join(output_dir, 'learned_rules.json'), 'r') as f:
        portalocker.lock(f, portalocker.LOCK_SH)
        rules = json.load(f)
        portalocker.unlock(f)
    observer_feedback = len(rules.get('corrections', [])) > 0
    
    with open(os.path.join(output_dir, 'arc_classification.json'), 'r') as f:
        portalocker.lock(f, portalocker.LOCK_SH)
        arc_data = json.load(f)
        portalocker.unlock(f)
    arc = arc_data['dominant_arc']
