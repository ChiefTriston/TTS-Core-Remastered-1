...
SILENCE_PAD_MS = config['silence_pad_ms']

model = WhisperModel("large-v3", device="cuda" if torch.cuda.is_available() else "cpu", compute_type="float16" if torch.cuda.is_available() else "int8")
    ...
    try:
        ...
        def process_speaker(label):
            try:
                ...
                with open(rttm_path, 'w') as rttm_f:
                    for idx, (start, end, _) in enumerate(slices):
                        if labels[idx] == label:
                            ...
                            try:
                                segments, info = model.transcribe(segment.numpy(), beam_size=5)
                                ...
                                text = ''
                                conf = 0
                            intervals.append({'start': start, 'end': end, 'text': text, 'conf': conf, 'certainty': certainties[idx]})
                            speaker_wave = torch.cat([speaker_wave, segment, silence])
                            total_dur += (end - start)
                            turn_count += 1
                            word_count += len(text.split())
                            rttm_f.write(f"SPEAKER {speaker_id} 1 {start:.3f} {(end - start):.3f} <NA> <NA> {speaker_id} <NA> <NA>\n")

                    for gap_start, gap_end, l1, l2, conf in overlaps:
                        if label in (l1, l2):
                            segment = waveform[0, int(gap_start * SAMPLE_RATE):int(gap_end * SAMPLE_RATE)]
                            try:
                                separated = separation_pipeline({"waveform": segment.unsqueeze(0), "sample_rate": SAMPLE_RATE})
                                speaker_seg = separated.get(label, segment)  # Improved
                            except Exception as e:
                                logging.error(f"Separation failed: {e}")
                                speaker_seg = segment
                            speaker_wave = torch.cat([speaker_wave, silence, speaker_seg])
                            intervals.append({'start': gap_start, 'end': gap_end, 'text': '[overlap]', 'overlap': True, 'conf': conf})
                            total_dur += (gap_end - gap_start)
                            rttm_f.write(f"SPEAKER {speaker_id} 1 {gap_start:.3f} {(gap_end - gap_start):.3f} <NA> <NA> {speaker_id} <NA> <NA>\n")

                if len(speaker_wave) == 0:
                    return None
                ...
                return speaker_id, wav_path, {'duration': total_dur, 'word_count': word_count, 'turn_count': turn_count}
            except Exception as e:
                logging.error(f"Error processing speaker {label}: {e}")
                return None

        with ThreadPoolExecutor(max_workers=8) as executor:  # Adaptive
