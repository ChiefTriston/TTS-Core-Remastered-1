...
MAX_SLICE_DUR = config['max_slice_dur']
MIN_MERGE_GAP = config['min_merge_gap']
...
def dynamic_slice(audio, device='cpu', embs=None, labels=None):  # Added embs and labels for speaker-aware
    try:
        ...
        # Learned fusion placeholder: simple average for now
        hybrid_probs = []
        for ts in speech_timestamps:
            ...
            hybrid_prob = (ts.get('probability', 1.0) + avg_webrtc) / 2  # TODO: Train fusion model
        ...
        slices = [(ts['start'], ts['end'], ts['prob']) for ts in speech_timestamps if MIN_SLICE_DUR <= (ts['end'] - ts['start']) <= MAX_SLICE_DUR and ts['prob'] > 0.5]
        
        # Speaker-aware merge if embs and labels provided
        merged = []
        if slices:
            current_start, current_end, current_prob = slices[0]
            current_label = labels[0] if labels is not None else None
            for i, (start, end, prob) in enumerate(slices[1:]):
