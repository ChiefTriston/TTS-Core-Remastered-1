...
import os
import pandas as pd
from .drift_utils import generate_segment_plot_map, save_drift_plot
...
def run(context):
    ...
    for speaker_id in speaker_ids:
        ...
        # Plot-map utilities
        # Stub full_scores with time and vader_compound (from tier1, but approximate with deltas_combined for now)
        full_scores = [{'time': t, 'vader_compound': deltas_combined[i] if i < len(deltas_combined) else 0} for i, t in enumerate(time)]
        boundaries = slice_starts  # Use starts as boundaries
        for b0, b1 in zip(boundaries, boundaries[1:]):
            generate_segment_plot_map(
                full_scores=full_scores, segment_start=b0, segment_end=b1,
                clip_id=f"{speaker_id}_{int(b0)}-{int(b1)}",
                tier1_trans="neutral→pos",  # compute from classify_tier1 runs
                drift_reason="high Δpitch",
                save_dir=os.path.join(speaker_out,"plot_maps")
            )
        save_drift_plot(full_scores, list(zip(boundaries, boundaries[1:])),
                os.path.join(speaker_out,"plot_maps","drift_overview.png"))
