...
Outputs drift_vector.json and drift_log.json, plus per‑slice plot maps.
"""
...
from modules.drift.drift import derive_segment_boundaries
...
def run(context):
    ...
    for spk in speakers:
        ...
        # --- compute deltas & rolling thresholds ---
        ...
        # --- raw drift points & boundaries ---
        drift_pts  = np.where((np.abs(delta_f0) > thresh_f0) | (np.abs(delta_energy) > thresh_energy))[0].tolist()
        ...
        polarity   = np.sign(combined[merged_drifts]) if merged_drifts else []
        pol_merged = []
        if merged_drifts:
            cp  = polarity[0]
            grp = [merged_drifts[0]]
        ...
        # --- compute time‑based slice boundaries ---
        ...
        for idx, start_idx in enumerate(filtered_drifts):
            ...
            generate_segment_plot_map(
                ...
                segment_start    = float(start_t),
                segment_end      = float(end_t),
                clip_id          = f"{spk}_{idx}",
                tier1_transition = "",
                drift_reason     = "",
                save_dir         = pm_dir
