...
logging.basicConfig(
    handlers=[logging.handlers.RotatingFileHandler('plot_map.log', maxBytes=10**6, backupCount=5)],
    level=logging.INFO,
...
def run(context):
    ...
    try:
        ...
        if pivot_times:
            ...
            pivot_idx = 0
            for beat in beats:
                while pivot_idx < len(pivot_times) and pivot_times[pivot_idx] < beat['start']:
                    pivot_idx += 1
                if pivot_idx < len(pivot_times) and beat['start'] < pivot_times[pivot_idx] < beat['end']:
                    # Split beat at pivot
                    ...
                    for start, end in drift.get('slice_boundaries', []):
                        try:
                            bid = next(
                                ...
                                if b['start'] <= start < b['end'] or
                                (b == beats[-1] and start >= b['start'])
                            )
