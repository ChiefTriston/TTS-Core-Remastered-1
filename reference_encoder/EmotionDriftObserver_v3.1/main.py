...
def enqueue_job(config, job_id, input_wav):
    ...
    # Load models required by the pipeline
    context['models'] = {}
    # Whisper ASR model for diarization & transcription
    ...
    # WhisperX align model (for segmentation alignment)
    ...
    # Store tuple matching diarization_run unpack
    context['models']['align'] = (align_model, metadata)

    # Run the full pipeline now that context is ready
    pipeline(context)
...
def main():
    ...
    if args.job:
        # Single-shot mode: enqueue and run immediately
        job_id = str(uuid.uuid4())
        descriptions[job_id] = 'queued'
        enqueue_job(config, job_id, args.job)
    elif args.watch:
        # Directory watching mode: start watcher and worker
